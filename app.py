...
# カスタムCSSによるデザイン（シャンパンゴールドのテーマ）
st.markdown("""
    <style>
    .main { background-color: #0e1117; }
    h1 { color: #D4AF37; font-family: 'Playfair Display', serif; font-weight: 700; }
    .stButton>button { background-color: #D4AF37; color: white; border-radius: 5px; font-weight: bold; width: 100%; border: none; padding: 12px; }
...
if st.button("EXECUTE PREMIER ANALYSIS", type="primary"):
    with st.spinner("AIがプレミアム・レポートを作成中です..."):
        try:
            ...
            2027年6月に日本へ帰国するオーナー（パークホームズ豊洲等の3LDK所有者を想定）向けに、
            以下の条件で市場分析を行ってください。
            【期間】{years[0]}-{years[1]}年 【エリア】{", ".join(areas)} 【タイプ】{prop_type} 【金利】{interest_rate}%
            1. 最初にCSV形式（時期,エリア名...,主な市場要因）を出力してください。
            2. その後「---SUMMARY---」と書き、2027年6月の帰国時に向けた資産価値予測と具体的アクションを日本語で詳しく解説してください。
            '''
            ...
            st.download_button(label="📥 Download Executive Data (CSV)", data=csv_buffer.getvalue(), 
                               file_name="wangan_premier_report.csv", mime="text/csv")

        except Exception as e:
            st.error(f"分析エラーが発生しました。条件を変更してください。({e})")
