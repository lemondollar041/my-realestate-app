...
st.markdown("<p style='color: #888;'>最新のAIアルゴリズムによる、湾岸エリア主要拠点の資産価値・市況推移の可視化</p>", unsafe_allow_html=True)
st.divider()
...
with st.sidebar:
    ...
    # 物件タイプの指定
    prop_type = st.radio("対象アセットを選択", ["標準（全体平均）", "3LDK / ファミリー向け（70㎡〜）", "1LDK / コンパクト（〜40㎡）"], index=0)
    ...
    with st.spinner("AIが市場トレンドを抽出しています..."):
        try:
            ...
            2. 次に「---SUMMARY---」と書き、その後に以下の2点について日本語で詳しく解説してください。
               A) 全体総括: {prop_type}における、金利{interest_rate}%が市場に与える影響と価格推移の背景。
               B) 将来展望: 各エリアの中長期的な資産価値の安定性と、現在推奨されるマーケットアクション。
            '''
